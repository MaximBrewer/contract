        ...
        <table cellpadding="0" cellspacing="0" class="s-table">
            ...
            <tbody>
                <tr>
                    <td>1</td>
                    <td>
                        {{ $auction->product->title }} по брони(ставке) {{ $bet->id }} аукцион номер {{ $auction->id }} с производством с {{ $interval->ffrom }} до {{ $interval->fto }}, частичная оплата({{ $auction->prepay }}% от суммы покупки)
                    
                    </td>
                    <td>{{ $auction->multiplicity->title }}</td>
                    <td>{{ $bet->volume }}</td>
                    <td>{{ $bet->correct * $auction->prepay / 100 * (float)$auction->multiplicity->coefficient }}</td>
                    <td>{{ $bet->correct * $auction->prepay / 100 * (float)$auction->multiplicity->coefficient * $bet->volume }}
                    </td>
                </tr>
                <tr class="result">
